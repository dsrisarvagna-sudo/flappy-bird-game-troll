...
// ----------------------------------------
// Responsive helper — true when screen width is phone-sized
...
// Clouds are initialised after resize() so canvas dimensions are known
let clouds = [];
...
function initClouds() {
    ...
    clouds = [
        { x: canvas.width * 0.15, y: canvas.height * 0.12 },
        { x: canvas.width * 0.50, y: canvas.height * 0.20 },
        { x: canvas.width * 0.82, y: canvas.height * 0.08 }
    ];
...
function resize() {
    ...
    // Re-spread clouds for new dimensions
    initClouds();
...
function addPipeWithCoin() {
    ...
    // Gap scales with playable height so it feels the same on any device.
    // Playable area = canvas.height - 70 (ground strip).
    // Clamp between 140 px (hard) and 220 px (easy) so it's fair on all screens.
    let gap = Math.min(220, Math.max(140, canvas.height * 0.28));

    let pipe = new Pipe(canvas.width, canvas.height, gap);
    pipes.push(pipe);
...
function drawScore() {
    ...
    // HUD base dimensions
    let hudW = isMobile() ? 150 : 220;
    let hudH = isMobile() ? 84  : 120;

    // Clamp scale so the panel (anchored at 15,15) never overflows right/bottom edge
    let maxScale = Math.min(
        (canvas.width  - 15) / hudW,
        (canvas.height - 15) / hudH
    );
    let clampedScale = Math.min(scoreScale, maxScale);
    ...
    ctx.lineWidth = 3;
    ctx.strokeRect(0, 0, hudW, hudH);

    // Text shadow
    ctx.shadowColor = "black";
    ctx.shadowBlur = 8;

    ctx.font = "bold " + (isMobile() ? "14px" : "24px") + " Arial";
    ctx.textAlign = "left";

    let hudLineH = isMobile() ? 26 : 38;

    ctx.fillStyle = "white";
    ctx.fillText("🏆 Score : " + score, 10, hudLineH);

    ctx.fillStyle = "#FFD700";
    ctx.fillText("🪙 Coins : " + coinsCollected, 10, hudLineH * 2 - 2);

    ctx.fillStyle = "#00FF66";
    ctx.fillText("⭐ Best : " + highScore, 10, hudLineH * 3 - 4);
...
function drawCountdown() {
    ...
    ctx.textAlign = "center";
    ctx.font = "bold " + (isMobile() ? "64px" : "110px") + " Arial";
    ...
    // Use 42% down — avoids the HUD panel above and ground below on short screens
    let countY = canvas.height * 0.42;
    ctx.strokeText(label, canvas.width / 2, countY);
    ctx.fillText(label,  canvas.width / 2, countY);
...
function drawPauseScreen() {
    ...
    ctx.font = "bold " + (isMobile() ? "40px" : "60px") + " Arial";
    ctx.fillStyle = "white";
    ctx.strokeStyle = "black";
    ctx.lineWidth = 5;
    ...
    ctx.font = (isMobile() ? "18px" : "28px") + " Arial";
    ctx.fillStyle = "rgba(255,255,255,0.85)";
    ctx.strokeStyle = "black";
    ctx.lineWidth = 3;
...
function drawGameOverScreen() {
    ...
    // ---- Layout: stack items from top of safe area to bottom ----
    // Safe area = above the ground (canvas.height - 70), with 5% padding top & bottom.
    let safeTop    = canvas.height * 0.05;
    let safeBottom = canvas.height - 70 - canvas.height * 0.03;
    let safeHeight = safeBottom - safeTop;

    // Row heights as fractions of safe area
    let rowTitle  = safeTop  + safeHeight * 0.14;   // title
    let rowScore  = safeTop  + safeHeight * 0.34;   // score
    let rowCoins  = safeTop  + safeHeight * 0.46;   // coins
    let rowBest   = safeTop  + safeHeight * 0.58;   // best
    let rowMedal  = safeTop  + safeHeight * 0.70;   // medal
    let rowBtn    = safeTop  + safeHeight * 0.80;   // button top
    let rowHint   = safeTop  + safeHeight * 0.97;   // "press R" hint
    ...
    ctx.fillStyle = "white";
    ctx.font = bodySize + "px Arial";
    ctx.fillText("🏆 Score : " + score,         cx, rowScore);
    ctx.fillText("🪙 Coins : " + coinsCollected, cx, rowCoins);

    ctx.fillStyle = "#FFD700";
    ctx.fillText("⭐ Best : " + highScore,       cx, rowBest);
    ...
    ctx.fillStyle = "white";
    ctx.fillText("Medal : " + medal, cx, rowMedal);

    // ---- RESTART BUTTON ----
    let btnW = Math.min(isMobile() ? 220 : 350, canvas.width * 0.6);
    let btnH = Math.min(isMobile() ? 50  : 70,  safeHeight * 0.14);
    let btnX = cx - btnW / 2;
    let btnY = rowBtn;

    // Guard: ensure button never goes below safe area
    if (btnY + btnH > safeBottom) {
        btnY = safeBottom - btnH;
    }
    ...
    ctx.fillStyle = "white";
    ctx.font = "bold " + Math.min(isMobile() ? 18 : 26, canvas.width / 18) + "px Arial";
    ctx.fillText("▶  RESTART", cx, btnY + btnH * 0.65);

    // Keyboard hint — only if there's room below the button
    if (rowHint < safeBottom) {
        ctx.font = Math.min(isMobile() ? 13 : 16, canvas.width / 38) + "px Arial";
        ctx.fillStyle = "rgba(255,255,255,0.5)";
        ctx.fillText("or press R", cx, rowHint);
    }
